...
// 데모 모드: Supabase 연결이 없을 때 localStorage로 동작
...
const SEED_REPORTS = [
  ...
  },
  {
    ...
    cause: '매출은 늘었지만 마진율이 -8%로 역전됐다. 쿠팡 수수료 11%, 광고비 2.3%, 무료배송 3.5%, 반품률 14% 처리비용까지 합치니 자사몰 마진 28%가 -8%로 깎였다. "쿠팡에 들어가면 돈 번다"는 통념이 1인 셀러에게는 함정이었다.',
  ...
  },
  {
    ...
    cause: '의사결정 구조 부재가 본질이었다. 50:50 지분은 "공정"한 게 아니라 "교착"이다. 갈등 해결 메커니즘(이사회·외부 자문) 없이 시작했고 18개월간 어떤 큰 결정도 합의되지 못했다. 결국 둘 다 지쳤다.',
    loss: '시드 투자금 4,200만원 소진(인건비). 18개월 시간. 둘 다 회사 떠남. 친구 관계도 끝남.',
    lesson: '공동창업 시 지분 차이를 반드시 둘 것 (예: 51:49). 의사결정 갈등 시 시니어 이사·외부 자문 회부 조항을 주주간계약서에 명시. "친구라서 괜찮을 것"이라는 가정은 가장 위험하다.',
    price: 28000,
    score: 91,
    grade: 'S',
...
if (typeof window !== 'undefined' && !isSupabaseConnected()) {
  ...
  if (existing.length === 0) {
    setDemoReports(SEED_REPORTS)
  }
}

export async function listReports({ category = null, sortBy = 'recent' } = {}) {
...
export async function createReport(report) {
  ...
  if (isSupabaseConnected()) {
    ...
    const { data, error } = await supabase
      .from('reports').insert(payload).select().single()
    if (error) throw error
    return data
  }
  const reports = getDemoReports()
